...
if __name__ == '__main__':
    ...
    # todo: 改数据格式
    for data_key in ['train', 'dev', 'test']:
    ...
    for step in tqdm(range(1, max_step + 1)):
        ...
        """
        word_seq_tensor: turn的tensor
        tag_seq_tensor: tag的tensor
        intent_tensor: intent的tensor
        word_mask_tensor: 
        tag_mask_tensor:
        context_seq_tensor:
        context_mask_tensor:
        """
        word_seq_tensor, intent_tensor, word_mask_tensor, context_seq_tensor, context_mask_tensor = batched_data
        if not config['model']['context']:
            context_seq_tensor, context_mask_tensor = None, None
        intent_logits, intent_loss = model.forward(word_seq_tensor, word_mask_tensor,
        ...
        if step % check_step == 0:
            ...
            val_intent_loss = 0
            model.eval()
            for pad_batch, ori_batch, real_batch_size in dataloader.yield_batches(batch_size, data_key='dev'):
